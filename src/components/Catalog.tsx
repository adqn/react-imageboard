...
const OpPost = ({ post }: { post: Post }) => {
  ...
  if (Number(fileWidth) > 150 || Number(fileHeight) > 150) {
    ratio = (fileWidth > fileHeight ? 150 / Number(fileWidth) : 150 / Number(fileHeight));
  ...
  return (
    <div className="catalog opPost">
      ...
      <span className="catalog threadInfo">P: {posts} / I: {images} / 0</span>
...
const Catalog = ({ uri }: { uri: string }) => {
  ...
  return (
    <div>
      ...
      <hr />
      <div className="navLinks">[<a href="../../">Home</a>] [<a href={"/" + uri}>Index</a>] [<a href="#update"
        onClick={() => getThreads()}>Update</a>]</div>
      <hr />
      <div className="catalog container">
        {mappedThreads ? mappedThreads.map((post: any) => <OpPost post={post} />) : null}
      </div>
    </div>
  )
}
